...
def build_answer(
    ...
    yield chat_history, "", avatar_html("talking")
    ...
    for ch in str(response):
        partial += ch
        preview = chat_history + [(question, partial)]
        yield preview, "", avatar_html("talking")
...
def create_ui(llm: ChatOllama, vector_store: Chroma, prompt: PromptTemplate):
    with gr.Blocks(title="Lokaler Unternehmens-Chatbot") as demo:
        ...
        with gr.Row():
            ...
            chatbot = gr.Chatbot(
                value=[("", "Hallo! Wie kann ich helfen?")],
                height=420,
                label="Chat",
        ...
        send.click(
            fn=lambda user_msg, history: build_answer(
                user_msg, history, llm=llm, vector_store=vector_store, prompt=prompt
            ),
            inputs=[msg, chatbot],
...
def main():
    ...
    ui.queue()
    ui.launch(server_name=args.host, server_port=args.port, show_error=True, share=False)
